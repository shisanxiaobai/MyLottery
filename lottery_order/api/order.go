...
func (o *Order) OrderList(ctx context.Context, in *proto.ListRequest, out *proto.ListResponse) error {
	orders, count, err := dao.OrderList(int(in.PageNum), int(in.PageSize))
	if err != nil {
		fmt.Println(err)
		out.Code = 500
		out.Msg = "获取订单列表失败！"
...
func (o *Order) OrderUser(ctx context.Context, in *proto.UserRequest, out *proto.UserResponse) error {
	orders, count, err := dao.OrderByEmail(in.Email, int(in.PageNum), int(in.PageSize))
	if err != nil {
		out.Code = 500
		out.Msg = "获取用户订单失败"
		return err
