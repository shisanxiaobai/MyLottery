<template>
  <div>
    <vs-navbar shadow-scroll fixed center-collapsed not-line>
      ...
      <template #right>
        <vs-button :active="active" @click="active = true" v-if="!is_login">
          登录
        </vs-button>
        <vs-navbar-item>
    ...
    <vs-sidebar v-model="activeItem" :open.sync="activeSidebar">
      ...
      <vs-sidebar-item :active="active" @click.native="Logout">
        退出
      </vs-sidebar-item>
    </vs-sidebar>
...
export default {
  ...
  methods: {
    ...
    async Postregister() {
      ...
      const { data: res } = await this.$http.post('user/registry', form_data)
      if (res.code == 200) {
        this.active2 = false
      }
      return alert(res.msg)
